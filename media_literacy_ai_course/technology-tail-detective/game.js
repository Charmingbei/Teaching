...
const defaultCloudEndpoint = "https://script.google.com/macros/s/AKfycbyIc0XirLYcM7q2tY7Bn0MWQAVWIYu7SJ4VIREdmZjF3O9nxlMiBCOxAn2YRme3xi1ebw/exec";
let activeGroup = 0;
...
function loadLessonRecord() {
  try {
    ...
    return saved ? { date: getToday(), className: "", cloudEndpoint: defaultCloudEndpoint, ...JSON.parse(saved) } : { date: getToday(), className: "", cloudEndpoint: defaultCloudEndpoint };
  } catch {
    return { date: getToday(), className: "", cloudEndpoint: defaultCloudEndpoint };
  }
}
...
async function uploadCloudBackup() {
  ...
  const endpoint = lessonRecord.cloudEndpoint?.trim() || defaultCloudEndpoint;
  if (!endpoint) {
  ...
  try {
    await Promise.race([upload, new Promise((resolve) => setTimeout(resolve, 4500))]);
